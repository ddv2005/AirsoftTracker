...
int MPU9250::readSensor() {
  ...
  _hrx = ((float)(_hxcounts) * _magScaleX);
  _hry = ((float)(_hycounts) * _magScaleY);
  _hrz = ((float)(_hzcounts) * _magScaleZ);

  _hrax = (_hrax*((float)_coeff-1)+_hrx)/((float)_coeff);
  _hray = (_hray*((float)_coeff-1)+_hry)/((float)_coeff);
  _hraz = (_hraz*((float)_coeff-1)+_hrz)/((float)_coeff);

  if(_autoMag)
  {
    updateMagMinMax(_hrax,_hray,_hraz);
    recalibrateMag();
  }
...
float MPU9250::getRawMagX_uT() {
  return _hrx;
}

float MPU9250::getRawMagY_uT() {
  return _hry;
}

float MPU9250::getRawMagZ_uT() {
  return _hrz;
}
...
int MPU9250FIFO::readFifo() {
  ...
  return _fifoSize/_fifoFrameSize;
}
...
int MPU9250::calibrateMag() {
  ...
  readSensor();
  _hxmax = getRawMagX_uT();
  _hxmin = getRawMagX_uT();
  _hymax = getRawMagY_uT();
  _hymin = getRawMagY_uT();
  _hzmax = getRawMagZ_uT();
  _hzmin = getRawMagZ_uT();
  ...
  while (_counter < _maxCounts) {
    ...
    _framedelta = 0.0f;
    readSensor();
    _hxfilt = (_hxfilt*((float)_coeff-1)+getRawMagX_uT())/((float)_coeff);
    _hyfilt = (_hyfilt*((float)_coeff-1)+getRawMagY_uT())/((float)_coeff);
    _hzfilt = (_hzfilt*((float)_coeff-1)+getRawMagZ_uT())/((float)_coeff);
...
void MPU9250::getMagMinMax(float &minX, float &maxX, float &minY, float &maxY, float &minZ, float &maxZ)
{
  minX = _hxmin;
  maxX = _hxmax;
...
void MPU9250::setMagMinMax(float minX, float maxX, float minY, float maxY, float minZ, float maxZ)
{
  _hxmin = minX;
  _hxmax = maxX;
...
void MPU9250::recalibrateMagByBias()
{
  _hxs = max(_hxmax-_hxb,_hxb-_hxmin);
  _hys = max(_hymax-_hyb,_hyb-_hymin);
  _hzs = max(_hzmax-_hzb,_hzb-_hzmin);
...
void MPU9250::updateMagMinMax(float mx, float my, float mz)
{
    if (mx > _hxmax) {
      _hxmax = mx;
    }
